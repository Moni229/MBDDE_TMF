"""Orquestación del pipeline de Silver"""
...
class SilverPipeline:
    """Transforma tablas Delta de Bronze en tablas Delta de Silver"""

    def __init__(self, spark, config_file_name: str = CONFIG_PIPELINE_FILE):
    ...
    def run(
        ...
        print(f"=== SILVER DF {datetime.now()} ===")
        ...
        target_table = f"{get_schemas()[self.layer]}.{table_name}"
        target_path = f"{get_layer_root(self.layer)}/{datasource}/{dataset}"
        query_name = f"{self.layer}-{datasource}-{dataset}"
